...
# NOTE     : To see the base script that was provided, please see "script2.py"
import time
import threading
import asyncio
...
async def main_async(list1, list2, list3):  # Main wrapper for asynchronous example
  s = time.perf_counter()
  # your code goes here
  tasks = list[cal_average_async(list1), cal_average_async(list2), cal_average_async(list3)]
...
def main_multiprocessing(list1, list2, list3):  # Main wrapper for multiprocessing example
  ...
  processes = []
  for list in lists:
    p = Process(target=cal_average, args=(list,))
    processes.append(p)
  for p in processes:
    p.start()
  for p in processes:
